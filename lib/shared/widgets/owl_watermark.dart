...
class OwlWatermark extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: width,
      height: height,
      child: CustomPaint(painter: _OwlPainter(color: color)),
    );
  }
}
...
class _OwlPainter extends CustomPainter {
  final Color color;
  _OwlPainter({required this.color});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(lw, Paint()..color = color.withOpacity(0.05));

    final rw = Path()
      ..moveTo(w*0.90,h*0.42)..quadraticBezierTo(w,h*0.65,w*0.92,h*0.82)
      ..quadraticBezierTo(w*0.82,h*0.70,w*0.78,h*0.55)..close();
    canvas.drawPath(rw, Paint()..color = color.withOpacity(0.05));

    final le = Path()
      ..moveTo(w*0.25,h*0.28)..lineTo(w*0.20,h*0.10)..lineTo(w*0.35,h*0.22)..close();
    canvas.drawPath(le, Paint()..color = color.withOpacity(0.08));

    final re = Path()
      ..moveTo(w*0.75,h*0.28)..lineTo(w*0.80,h*0.10)..lineTo(w*0.65,h*0.22)..close();
    canvas.drawPath(re, Paint()..color = color.withOpacity(0.08));

    final bico = Path()
      ..moveTo(w*0.42,h*0.46)..lineTo(w*0.50,h*0.54)..lineTo(w*0.58,h*0.46)..close();
    canvas.drawPath(bico, Paint()..color = color.withOpacity(0.07));

    final pp = Paint()..color = color.withOpacity(0.05)..strokeWidth = 1.2
    ...
    for (final cx in [0.33, 0.67]) {
      ...
      final r = w*0.155;
      canvas.drawCircle(c, r, Paint()..color = color.withOpacity(0.22));
      canvas.drawCircle(c, r, Paint()
        ..color = color.withOpacity(0.15)
        ..style = PaintingStyle.stroke..strokeWidth = 1.0);
      canvas.drawCircle(c, r*0.45, Paint()..color = color.withOpacity(0.10));
    }
  }

  @override
  bool shouldRepaint(_OwlPainter old) => old.color != color;
}
