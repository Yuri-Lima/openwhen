...
| Badges / gamificação (10 conquistas + escudos)                                     | P1         | Concluído | Médio          | 10 conquistas em `lib/features/gamification/`: marcos (1.ª carta enviada/aberta/recebida), social (feed público, 10 curtidas), volume (5/10 cartas), especial (voz, perfil completo, streak 3 dias). Visual de escudos/brasões (`CustomPainter`) com cores por categoria e adaptação a 4 temas. Bottom sheet com detalhes. Hooks em `badge_unlock_service.dart`. Campos streak: `lastActiveDate`, `currentStreak`. Firestore rules allowlist. Ver [ARCHITECTURE.md](ARCHITECTURE.md) secção "Gamificação". |
