...
- **App Check enforced em todas as Cloud Functions (2026-05-03):** `enforceAppCheck: true` adicionado a todas as callable Cloud Functions (admin, moderação, billing, share link, delete account, export, check username, backfill, log access). Requisições sem token App Check válido são rejeitadas com 403.
- **App Check iOS corrigido (2026-05-03):** removido `if (Platform.isIOS) return` em `_activateAppCheckIfNeeded()` — App Check agora ativa em todas as plataformas. `SafeCallable` (HTTP fallback iOS) atualizado para enviar `X-Firebase-AppCheck` header manualmente, contornando o bug firebase-ios-sdk#15974. Token obtido com timeout de 8s para evitar deadlock.
- **Blocking function anti-abuse (2026-05-03):** nova Cloud Function `onUserCreated` (`beforeUserCreated` trigger) bloqueia criação de conta antes de completar no Firebase Auth. Verifica: (1) emails descartáveis (30+ domínios bloqueados), (2) rate limiting por IP (máx 5 contas/24h via `accountCreationLogs`). Requer Identity Platform (ativado no Console).
- **Rate limiting em `checkUsernameAvailable` (2026-05-03):** rate limiter in-memory (30 req/min/IP) com cleanup automático a 1500 IPs. `extractIp` prioriza `rawRequest.ip` sobre `x-forwarded-for` para prevenir spoofing.
- **Firestore rules reforçadas para criação de utilizador (2026-05-03):** `users/{userId}` create exige `createdAt == request.time` (server timestamp), `username` string 3-20 chars, `dateOfBirth` timestamp, `acceptedTermsVersion` e `acceptedPrivacyVersion` strings, `termsAcceptedAt` timestamp, `subscriptionTier == 'free'`. Coleção `accountCreationLogs` com acesso bloqueado (Admin SDK only).
- **Timestamps server-side obrigatórios (2026-05-03):** `Timestamp.now()` substituído por `FieldValue.serverTimestamp()` em `createdAt` e `termsAcceptedAt` nos 3 caminhos de criação de conta (register, Apple, Google) em `auth_repository.dart`.
- **`ensureUserFirestoreProfileExists` robusto (2026-05-03):** sanitização completa de username (regex `[a-z0-9._]`, strip dots nas bordas, min 3 chars com fallback `user.XXXXXX`), campos obrigatórios adicionados (`dateOfBirth` placeholder, `acceptedTermsVersion`, `acceptedPrivacyVersion`, `termsAcceptedAt`), flag `profileIncomplete: true` para UI detectar perfis incompletos.

### Added

- **Compartilhar carta via link (Share via Link):** terceira opção de envio ("Gerar link") em `write_letter_screen.dart`. Remetente gera URL partilhável (`whenote.app/open/{token}`) via callable `generateShareLink`; destinatário abre landing page web com preview emocional (nome remetente, data, emoção — sem conteúdo), faz download do app e claim automático via `claimShareLink` (dentro de transaction Firestore). 4 Cloud Functions: `generateShareLink`, `getSharePreview` (HTTP público com rate limiting 60 req/min/IP), `claimShareLink`, `revokeShareLink`. Dual limit anti-abuse (100 activos + 500 total por utilizador). Token collision check. Landing page dinâmica (`hosting/public/open/index.html`) com estados loading/preview/claimed/revoked/rate-limited/error + OG meta tags. Deep link handling para `/open/{token}` em `DeepLinkCoordinator` + `PendingDeepLink`. Banner de estado do link na `letter_detail_screen.dart` (pending/claimed/revoked). FCM push + notificação in-app ao remetente no claim. Intent filter Android + AASA iOS para `/open/*`. Firebase Hosting rewrite `/api/share-preview` → Cloud Function. Novos campos em `letters`: `shareToken`, `shareMode`, `shareCreatedAt`, `shareClaimedAt`, `shareClaimedBy`, `shareRevoked`. 3 índices compostos. 16 chaves i18n (en/pt/pt-BR/es). Plano completo: `planning/SHARE_VIA_LINK.md`.
- **Rascunhos de carta (Drafts):** nova coleção Firestore `drafts` com save manual (botão na AppBar + dialog ao sair), TTL Policy no campo `expiresAt` (30 dias, deleção automática server-side). `DraftService` com CRUD, limpeza client-side de expirados, limite de 10 drafts/utilizador (`draftCount` no user doc), e migração one-time do SharedPreferences legado. `DraftsScreen` com listagem, emoji badges, barra de countdown (cores por urgência), swipe-to-delete com confirmação. Navegação via ícone no header do WriteLetterScreen. 17 chaves i18n (en/pt/pt-BR/es) com plurais ICU. Novos ficheiros: `draft_model.dart`, `draft_service.dart`, `drafts_screen.dart`, `emotional_state.dart` (extraído para resolver dependência circular). Regras Firestore para `drafts/{draftId}` com proteção de campos imutáveis.
